package com.ruoyi.kaisheng.controller;

import java.util.List;
import org.apache.shiro.authz.annotation.RequiresPermissions;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.ModelMap;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseBody;
import com.ruoyi.common.annotation.Log;
import com.ruoyi.common.enums.BusinessType;
import com.ruoyi.kaisheng.domain.KsDownbound;
import com.ruoyi.kaisheng.service.IKsDownboundService;
import com.ruoyi.common.core.controller.BaseController;
import com.ruoyi.common.core.domain.AjaxResult;
import com.ruoyi.common.utils.poi.ExcelUtil;
import com.ruoyi.common.core.page.TableDataInfo;

/**
 * 南下表單Controller
 * 
 * @author Sheng
 * @date 2022-02-19
 */
@Controller
@RequestMapping("/ks/downbound")
public class KsDownboundController extends BaseController
{
    private String prefix = "ks/downbound";

    @Autowired
    private IKsDownboundService ksDownboundService;

    @RequiresPermissions("ks:downbound:view")
    @GetMapping()
    public String downbound()
    {
        return prefix + "/downbound";
    }

    /**
     * 查询南下表單列表
     */
    @RequiresPermissions("ks:downbound:list")
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(KsDownbound ksDownbound)
    {
        startPage();
        List<KsDownbound> list = ksDownboundService.selectKsDownboundList(ksDownbound);
        return getDataTable(list);
    }

    /**
     * 导出南下表單列表
     */
    @RequiresPermissions("ks:downbound:export")
    @Log(title = "南下表單", businessType = BusinessType.EXPORT)
    @PostMapping("/export")
    @ResponseBody
    public AjaxResult export(KsDownbound ksDownbound)
    {
        List<KsDownbound> list = ksDownboundService.selectKsDownboundList(ksDownbound);
        ExcelUtil<KsDownbound> util = new ExcelUtil<KsDownbound>(KsDownbound.class);
        return util.exportExcel(list, "南下表單数据");
    }

    /**
     * 新增南下表單
     */
    @GetMapping("/add")
    public String add()
    {
        return prefix + "/add";
    }

    /**
     * 新增保存南下表單
     */
    @RequiresPermissions("ks:downbound:add")
    @Log(title = "南下表單", businessType = BusinessType.INSERT)
    @PostMapping("/add")
    @ResponseBody
    public AjaxResult addSave(KsDownbound ksDownbound)
    {
        return toAjax(ksDownboundService.insertKsDownbound(ksDownbound));
    }

    /**
     * 修改南下表單
     */
    @RequiresPermissions("ks:downbound:edit")
    @GetMapping("/edit/{id}")
    public String edit(@PathVariable("id") Integer id, ModelMap mmap)
    {
        KsDownbound ksDownbound = ksDownboundService.selectKsDownboundById(id);
        mmap.put("ksDownbound", ksDownbound);
        return prefix + "/edit";
    }

    /**
     * 修改保存南下表單
     */
    @RequiresPermissions("ks:downbound:edit")
    @Log(title = "南下表單", businessType = BusinessType.UPDATE)
    @PostMapping("/edit")
    @ResponseBody
    public AjaxResult editSave(KsDownbound ksDownbound)
    {
        return toAjax(ksDownboundService.updateKsDownbound(ksDownbound));
    }

    /**
     * 删除南下表單
     */
    @RequiresPermissions("ks:downbound:remove")
    @Log(title = "南下表單", businessType = BusinessType.DELETE)
    @PostMapping( "/remove")
    @ResponseBody
    public AjaxResult remove(String ids)
    {
        return toAjax(ksDownboundService.deleteKsDownboundByIds(ids));
    }
}
